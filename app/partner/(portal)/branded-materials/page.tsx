...
export default async function PartnerBrandedMaterialsPage() {
  ...
  const resources = all.filter((r) => BRANDED_CATEGORIES.includes(r.category as string))
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight flex items-center gap-2"><Palette className="w-6 h-6" /> Branded Materials</h2>
        <p className="text-muted-foreground">Brochures, badges, partner logos and brand assets for your website.</p>
      </div>

      <Card>
        <CardHeader><CardTitle className="text-sm font-medium">Available Downloads</CardTitle></CardHeader>
        <CardContent>
          {resourcesWithUrls.length === 0 ? <p className="text-sm text-muted-foreground">No branded materials available yet.</p> : (

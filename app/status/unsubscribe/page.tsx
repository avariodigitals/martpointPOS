...
function UnsubscribeForm() {
  ...
  const unsubscribe = async () => {
    setState("loading")
    try {
      const res = await fetch("/api/status/unsubscribe", {
        method: "POST",
        ...
        {state === "done" ? (
          <>
            ...
            <p className="text-sm text-muted-foreground">
              You will no longer receive MartPoint status update emails.
            </p>
          </>
        ) : (
          <>
            <Bell className="w-12 h-12 text-muted-foreground mx-auto" />
            <h1 className="text-xl font-semibold">Unsubscribe from status updates</h1>
            {token ? (
              <>
                <p className="text-sm text-muted-foreground">
                  Click below to stop receiving incident and maintenance emails from MartPoint.
                </p>
...
export default function StatusUnsubscribePage() {
  return (
